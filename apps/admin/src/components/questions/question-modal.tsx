...
export function QuestionModal({
  ...
  const getInitialChoices = (): string[] => {
    const c = question.choices as Choice[];
    return c.map(ch => ch.text);
  };

  const getInitialCorrectIndex = (): string => {
    const c = question.choices as Choice[];
    const idx = c.findIndex(ch => ch.id === question.correctAnswerId);
  ...
  const [choices, setChoices] = useState<string[]>(getInitialChoices);
  const [correctIndex, setCorrectIndex] = useState<string>(getInitialCorrectIndex);
  ...
  if (open !== prevOpen) {
    ...
    if (open) {
      ...
      setChoices(getInitialChoices());
      setCorrectIndex(getInitialCorrectIndex());
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-4">
          ...
          <div className="space-y-2">
            ...
            <Popover open={comboboxOpen} onOpenChange={setComboboxOpen}>
              <PopoverTrigger className={cn("w-full justify-between inline-flex items-center rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background")} role="combobox" aria-expanded={comboboxOpen}>
