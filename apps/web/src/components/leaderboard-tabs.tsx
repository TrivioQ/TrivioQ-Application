      ...
      <div className="flex flex-col items-center gap-6">
        ...
        {isLoggedIn && (
          <div className="flex items-center p-1 bg-white/40 dark:bg-white/5 rounded-full border border-white dark:border-white/10 backdrop-blur-xl shadow-xl shadow-brand-900/10 dark:shadow-none">
            <button onClick={() => setActiveMode('global')} className={`px-5 py-1.5 rounded-full text-xs font-bold transition-all ${activeMode === 'global' ? 'bg-brand-500 text-white shadow-md' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}`}>
              {t('global')}
            </button>
            <button onClick={() => setActiveMode('friends')} className={`px-5 py-1.5 rounded-full text-xs font-bold transition-all ${activeMode === 'friends' ? 'bg-brand-500 text-white shadow-md' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}`}>
              {t('friends')}
            </button>
          </div>
        )}
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      <div className="relative group/tooltip">
                        <span className="font-bold text-gray-900 dark:text-white text-lg cursor-help border-b border-dashed border-gray-400 dark:border-gray-600 hover:border-brand-500 dark:hover:border-brand-400 transition-colors">{user.displayName || user.username}</span>
                        {/* Tooltip */}
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-1.5 bg-gray-800 text-xs text-white dark:text-brand-300 rounded-lg opacity-0 group-hover/tooltip:opacity-100 transition-opacity pointer-events-none border border-gray-700 dark:border-brand-500/30 whitespace-nowrap z-50 shadow-xl">
                          @{user.username}
