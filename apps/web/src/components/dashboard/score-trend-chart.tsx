...
const CustomTooltip = ({ active, payload, label, mode }: any) => {
  ...
  const title = mode === 'monthly' ? d.toLocaleDateString('en-US', { month: 'long', year: 'numeric', timeZone: 'UTC' }) : `Week of ${d.toLocaleDateString('en-US', { month: 'short', day: 'numeric', timeZone: 'UTC' })}`;
...
export function ScoreTrendChart({ data, mode }: Props) {
  ...
  if (sorted.length === 0) {
    return <div className='flex h-[220px] items-center justify-center text-sm text-gray-500'>No data yet.</div>;
  }
  ...
  return (
    <ResponsiveContainer width='100%' height={220}>
      <BarChart data={sorted} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
        ...
        <XAxis dataKey='periodStart' tick={{ fontSize: 10, fill: '#6b7280' }} tickFormatter={(v) => formatLabel(v, mode)} interval='preserveStartEnd' />
