...
function UserAvatar({ profilePicture, displayName, email }: { profilePicture: string | null; displayName: string | null; email: string }) {
  if (profilePicture) {
    return <img src={profilePicture} alt='avatar' className='w-8 h-8 rounded-full object-cover ring-2 ring-indigo-500/60' />;
  }
  return <div className='w-8 h-8 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-xs font-bold ring-2 ring-indigo-500/60'>{getInitials(displayName, email)}</div>;
}
