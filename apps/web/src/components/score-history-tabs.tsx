    ...
    <div className="space-y-6">
      <div className="flex justify-center p-1 bg-white/40 dark:bg-gray-900/50 rounded-xl border border-white dark:border-white/5 backdrop-blur-xl w-fit mx-auto shadow-xl shadow-brand-900/10 dark:shadow-none">
        {(['weekly', 'monthly'] as const).map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`px-8 py-2.5 rounded-lg text-sm font-bold transition-all duration-200 ${activeTab === tab ? 'bg-brand-500 text-white shadow-lg shadow-brand-500/20' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
