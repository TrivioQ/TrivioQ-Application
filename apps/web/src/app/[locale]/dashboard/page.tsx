...
export default async function WebDashboard() {
      ...
      <div className="min-h-screen flex items-center justify-center px-6 text-gray-900 dark:text-white">
        <div className="text-center space-y-4">
          ...
          <p className="text-gray-500 dark:text-gray-400">
            {t('signInPrompt')}{' '}
            <Link href="/login" className="text-blue-600 hover:text-blue-500 dark:text-teal-400 dark:hover:text-teal-300">
              {t('signInLink')}
            </Link>{' '}
            {t('signInSuffix')}
          </p>
  ...
  return (
    <div className="min-h-screen text-gray-900 dark:text-white selection:bg-blue-500 dark:selection:bg-teal-500 selection:text-white">
      <div className="max-w-5xl mx-auto px-6 py-24 space-y-10">
        {/* ── Header ── */}
        <div>
          <h1 className="text-4xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-blue-500 to-orange-500 dark:from-teal-400 dark:via-purple-400 dark:to-pink-400">{profile ? t('greeting', { name: profile.displayName ?? profile.username }) : t('title')}</h1>
        ...
        <div className="flex flex-wrap gap-3 pt-2">
          <Link href="/score-history" className="rounded-xl bg-teal-600 hover:bg-teal-500 dark:bg-teal-600/20 dark:hover:bg-teal-600/30 border border-teal-600 dark:border-teal-500/30 px-4 py-2 text-sm text-white dark:text-teal-300 font-medium transition-colors">
            {t('fullScoreHistory')}
          </Link>
