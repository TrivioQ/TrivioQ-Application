...
export default async function PrivacyPage() {
  ...
  return (
    <main className="min-h-screen bg-bg-primary text-text py-12 px-4">
      <div className="max-w-3xl mx-auto">{doc ? <article className="prose prose-invert prose max-w-none" dangerouslySetInnerHTML={{ __html: marked(doc.content) }} /> : <p className="text-text-muted text-center">{t('privacyUnavailable')}</p>}</div>
    </main>
  );
}
