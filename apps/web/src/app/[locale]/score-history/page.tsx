  ...
  return (
    <div className="min-h-screen py-20 px-6 sm:px-8 text-gray-900 dark:text-white selection:bg-brand-500 selection:text-gray-900 dark:text-white">
      <div className="max-w-4xl mx-auto space-y-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <div key={d.label} className={`rounded-2xl bg-gradient-to-br ${d.color} border p-5 text-center`}>
              <p className="text-xs uppercase tracking-widest font-bold text-gray-600 dark:text-gray-400 mb-1">{t('difficultyLabel')}</p>
              <p className={`text-xl font-extrabold ${d.color.split(' ')[4]}`}>{d.label}</p>
              <p className="text-3xl font-black text-gray-900 dark:text-white mt-1">{d.pts}</p>
            </div>
          ))}
        </div>
        ...
        <div className="rounded-2xl border border-brand-500/20 bg-brand-500/5 p-6 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-600 dark:text-gray-400">
            <div>
              ...
              <p className="text-xs mt-1 text-gray-500 dark:text-gray-600 dark:text-gray-400">{t('weeklyBonusNote')}</p>
            </div>
            <div>
              <p className="font-semibold text-gray-900 dark:text-white mb-1">{t('monthlyTop10')}</p>
              <p>{t('monthlyBonusDesc')}</p>
              <p className="text-xs mt-1 text-gray-500 dark:text-gray-600 dark:text-gray-400">{t('monthlyBonusNote')}</p>
            </div>
          </div>
        </div>
        ...
        {/* ── Main table ── */}
        {!hasCookie ? (
          <div className="py-20 text-center text-gray-500 dark:text-gray-600 dark:text-gray-400">
