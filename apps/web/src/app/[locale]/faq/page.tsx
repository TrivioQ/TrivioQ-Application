...
export default async function FAQPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white selection:bg-teal-500 selection:text-white pb-20">
      {/* ── Hero Section ── */}
      <div className="relative overflow-hidden pt-20 pb-16 px-6">
        ...
        <div className="max-w-4xl mx-auto text-center space-y-4">
          <h1 className="text-5xl md:text-6xl font-black tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-white via-teal-200 to-white">{t('title')}</h1>
          ...
          faqs.map((faq) => (
            <details key={faq.id} className="group bg-gray-900/50 border border-white/5 rounded-3xl overflow-hidden hover:border-white/10 transition-all duration-300">
              <summary className="flex items-center justify-between p-6 md:p-8 cursor-pointer list-none">
                <span className="text-lg md:text-xl font-bold pr-6 group-open:text-teal-400 transition-colors">{faq.question}</span>
      ...
      <div className="max-w-4xl mx-auto px-6 mt-20">
        <div className="bg-gradient-to-br from-teal-600/20 to-purple-600/20 border border-teal-500/20 rounded-[2.5rem] p-10 md:p-16 text-center space-y-8 relative overflow-hidden group">
          ...
          <div className="space-y-4">
            <h2 className="text-3xl font-bold text-white">{t('stillHaveQuestions')}</h2>
            <p className="text-gray-400 max-w-lg mx-auto">{t('contactDesc')}</p>
          </div>
          <a href="mailto:support@trivioq.com" className="inline-flex items-center gap-2 bg-teal-500 hover:bg-teal-400 text-white font-bold py-4 px-10 rounded-2xl transition-all shadow-lg shadow-teal-500/25 hover:-translate-y-1 active:translate-y-0">
