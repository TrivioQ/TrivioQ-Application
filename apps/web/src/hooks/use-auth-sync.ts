...
export function useAuthSync({ onSuccess, onError }: AuthSyncOptions = {}) {
  ...
  const signupMutation = useMutation({
    mutationFn: ({ email, password, username, displayName }: { email: string; password: string; username: string; displayName: string }) => makeAPICall('/auth/signup', { method: 'POST', body: { email, password, username, displayName } }),
    onSuccess: handleSuccess,
    onError: handleError,
  });
